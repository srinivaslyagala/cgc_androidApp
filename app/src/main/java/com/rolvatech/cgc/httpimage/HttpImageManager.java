...
import com.rolvatech.cgc.webaccess.HttpHelper;
        ...
        return new Runnable() {
            ...
            public void run() {
                ...
                if (request.getImageView() != null) {
                    ...
                    synchronized (iv) {
                        if (iv.getTag() != request.getUri()) {
                            if (DEBUG)
                                Log.e(TAG, "give up loading: " + request.getUri().toString());
                            return;
                        }
                    }
                }
                ...
                try {
                    ...
                    if (data == null) {
                        if (DEBUG) Log.e(TAG, "cache missing " + request.getUri().toString());
                        //then check the persistent storage
                        data = mPersistence.loadData(key);
                        if (data != null) {
                            if (DEBUG)
                                Log.e(TAG, "found in persistent: " + request.getUri().toString());
                            ...
                            // we go to network
                            if (DEBUG)
                                Log.e(TAG, "go to network " + request.getUri().toString());
                            ...
                            try {
                                try {
                                    ...
                                    if (responseStream == null) {
                                        responseStream = new HttpHelper().sendRequest(request.path, "GET", null, null);
                                    }
                                }
                                if (responseStream != null) {
...
//                                         data = BitmapUtil.decodeByteArray(binary, mMaxNumOfPixelsConstraint);
                            ...
                            if (DEBUG)
                                Log.e(TAG, "decoded image: " + data.getWidth() + "x" + data.getHeight());
                            if (DEBUG)
                                Log.e(TAG, "time consumed: " + (System.currentTimeMillis() - millis));
                    ...
                    if (DEBUG) Log.e(TAG, "finished request for: " + request.getUri());
                }
            }
        };
    }
